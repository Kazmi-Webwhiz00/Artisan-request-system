...
/**
 * Render a Bootstrap-styled number field with prefixed classes for uniqueness
 *
 ...
 * @param string|int|float $value Default value (optional).
 * @param bool $required Whether the field is required (optional, default false).
 * @param array $additional_attrs Additional attributes like min, max, step (optional).
 */
function render_number_field($name, $id, $label = '', $placeholder = '', $value = '', $required = false, $additional_attrs = []) {
    // Start rendering
    ...
    // Add input field
    $field .= '<input type="number" class="form-control" name="' . esc_attr($name) . '" id="' . esc_attr($id) . '"';
    if ($placeholder) {
